...
namespace kebbet\cpt\explore\admincolumns;

use const kebbet\cpt\explore\POSTTYPE;
...
function set_admin_column_list( $columns ) {
	$columns['modified']  = __( 'Last modified', 'kebbet-cpt-explore' );
	$columns['thumbnail'] = __( 'Featured image', 'kebbet-cpt-explore' );
	return $columns;
}
...
function populate_custom_columns( $column, $post_id ) {
	...
	if ( 'thumbnail' === $column ) {
		...
		if ( $thumbnail ) {
			...
			echo __( 'No image is set.', 'kebbet-cpt-explore' );
		}
	}
}
